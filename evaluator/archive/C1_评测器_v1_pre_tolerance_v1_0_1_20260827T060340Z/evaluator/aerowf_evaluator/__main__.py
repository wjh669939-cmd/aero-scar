    ...
    for 机场, 机场数据 in 数据.items():
        ...
        for 预测位置, (跨度名, 分钟偏移) in enumerate(配置["forecast"]["horizons"].items()):
            ...
            for 分量位置, 分量名 in enumerate(配置["forecast"]["components"]):
                ...
                越界数 = int(((计分预测 < 配置["forecast"]["valid_range"][0]) | (计分预测 > 配置["forecast"]["valid_range"][1])).sum())
                if 越界数:
                    异常计数["out_of_range"] += 越界数
                    raise 提交无效("真实计分跑道预测越界，禁止 clip", 异常计数)
                误差 = 计分预测 - 跑道真值[伙伴位置[行], 跑道槽, 95, 分量位置 + 1]
...
def 评测插补(预测, 数据, 元信息, 私有配置, 配置, 异常计数):
    ...
    for 机场, 机场数据 in 数据.items():
        for 情景位置, 情景 in enumerate(配置["imputation"]["scenarios"]):
            ...
            计分预测 = 预测[机场][:, 情景位置]
            越界数 = int(((计分预测[有效掩码] < 配置["forecast"]["valid_range"][0]) | (计分预测[有效掩码] > 配置["forecast"]["valid_range"][1])).sum())
            if 越界数:
                异常计数["out_of_range"] += 越界数
                raise 提交无效("真实计分插补预测越界，禁止 clip", 异常计数)
            误差 = 计分预测 - 机场数据["runway"]
