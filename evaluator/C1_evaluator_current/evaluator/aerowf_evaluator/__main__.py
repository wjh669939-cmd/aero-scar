...
def 分类端点(名称, 真值, 预测, 块, 类别, 配置, 异常计数):
    ...
    # V2 只规定 zero-support 的 F1/CSI 为 NA。基率为 0 或 1 时，
    # 数值仍须如实报告；CLH 的退化规则只影响其是否参与 overall。
    if 零支持:
...
def 分类宏端点(名称, 真值, 预测, 块, 类别列表, 指标, 配置):
    ...
    可评基率 = []
    for 类别 in 类别列表:
        支持度 = int(np.sum(真值 == 类别))
        if 支持度 > 0:
            可评类别.append(类别)
            可评基率.append(float(支持度 / len(真值)))
    if not 可评类别:
    ...
    return {"name": 名称, "value": float(np.mean(完整值)), "ci95": 区间,
            "degenerate": all(基率 in (0.0, 1.0) for 基率 in 可评基率),
            "included_endpoint_count": len(可评类别)}
...
def 决策指标别名(总体, 配置, 任务):
    """将冻结 policy 使用的指标名显式映射到本次任务的 overall 输出。"""
    原指标 = {端点["name"]: 端点 for 端点 in 总体}
    结果 = {}
    for policy名称, overall名称 in 配置["decision_policy_metric_bindings"].get(任务, {}).items():
        if overall名称 not in 原指标:
            raise 评测故障(f"DecisionPolicy 指标映射缺少 overall：{overall名称}")
        端点 = 原指标[overall名称]
        结果[policy名称] = {"source_metric": overall名称, "value": 端点["value"],
                         "ci95": 端点["ci95"], "degenerate": 端点["degenerate"]}
    return 结果
...
def 校验计分位置值域(计分预测列表, 配置, 异常计数, 任务名):
    """只核验实际参与指标计算的位置；不修改任何预测值。"""
    常规下界, 常规上界 = 配置["forecast"]["valid_range"]
    容忍配置 = 配置["forecast"]["out_of_range_tolerance"]
    容忍下界, 容忍上界 = 容忍配置["allowed_range"]
    计分位置数 = sum(int(预测.size) for 预测 in 计分预测列表)
    越界值列表 = [预测[(预测 < 常规下界) | (预测 > 常规上界)] for 预测 in 计分预测列表]
    越界值列表 = [值 for 值 in 越界值列表 if 值.size]
    越界数 = sum(int(值.size) for 值 in 越界值列表)
    异常计数["out_of_range"] += 越界数
    if not 越界数:
        return

    越界值 = np.concatenate(越界值列表)
    # nextafter 只吸收 float32 的 1.1 等配置边界表示误差，不扩大数学容忍带。
    数据类型 = 越界值.dtype
    下界 = np.nextafter(np.asarray(容忍下界, dtype=数据类型), -np.inf)
    上界 = np.nextafter(np.asarray(容忍上界, dtype=数据类型), np.inf)
    if np.any((越界值 < 下界) | (越界值 > 上界)):
        raise 提交无效(f"{任务名}真实计分位置越界超出容忍带，禁止 clip", 异常计数)

    最大占比 = float(容忍配置["max_fraction_scoring_positions"])
    if 计分位置数 == 0 or 越界数 > 最大占比 * 计分位置数:
        raise 提交无效(f"{任务名}真实计分位置越界数量超过容忍阈值，禁止 clip", 异常计数)
    ...
    for 机场, 机场数据 in 数据.items():
        ...
        for 预测位置, (跨度名, 分钟偏移) in enumerate(配置["forecast"]["horizons"].items()):
            ...
            for 分量位置, 分量名 in enumerate(配置["forecast"]["components"]):
                计分预测 = 预测[机场][行, 跑道槽, 预测位置, 分量位置]
                计分条目.append((机场, 跨度名, 分量名, 分量位置, 行, 跑道槽, 伙伴位置, 计分预测, 块))
    校验计分位置值域([条目[7] for 条目 in 计分条目], 配置, 异常计数, "预测")
    for 机场, 跨度名, 分量名, 分量位置, 行, 跑道槽, 伙伴位置, 计分预测, 块 in 计分条目:
        误差 = 计分预测 - 数据[机场]["runway"][伙伴位置[行], 跑道槽, 95, 分量位置 + 1]
...
def 评测分类(预测, 数据, 标签组, 配置, 异常计数):
    ...
    总体 = [跨端点等权平均("overall.classification.macro_f1", [端点 for 端点 in 端点 if 端点["name"].endswith(".macro_f1")]),
            ...
            跨端点等权平均("overall.classification.f1_good", [端点 for 端点 in 端点 if 端点["name"].endswith(".f1_good")]),
            跨端点等权平均("overall.classification.f1_precip", [端点 for 端点 in 端点 if 端点["name"].endswith(".f1_precip")]),
            跨端点等权平均("overall.classification.f1_class_hazard", [端点 for 端点 in 端点 if 端点["name"].endswith(".f1_class_hazard")])]
    return 端点, 总体


def 评测插补(预测, 数据, 元信息, 私有配置, 配置, 异常计数):
    端点 = []
    计分条目 = []
    ...
    for 机场, 机场数据 in 数据.items():
        for 情景位置, 情景 in enumerate(配置["imputation"]["scenarios"]):
            ...
            计分预测 = 预测[机场][:, 情景位置]
            计分条目.append((机场, 情景, 有效掩码, 计分预测))
    校验计分位置值域([条目[3][条目[2]] for 条目 in 计分条目], 配置, 异常计数, "插补")
    for 机场, 情景, 有效掩码, 计分预测 in 计分条目:
        误差 = 计分预测 - 数据[机场]["runway"]
        每样本数量 = 有效掩码.reshape(有效掩码.shape[0], -1).sum(axis=1)
        块 = np.repeat(日期块编号(数据[机场]["timestamps"]), 每样本数量)
...
def 构造清单(参数, 私有配置, 配置, 样本数, 任务):
    ...
    return {"utc": utc时间(), "evaluator_version": 配置["evaluator_version"],
            ...
            "data_release": 配置["data_release"], "metric_scale": 配置["metric_scale"],
            "forecast_protocol": 配置["forecast_protocol"] if 任务 == "forecast" else None,
...
def 运行(参数):
    ...
    if 任务 == "forecast":
        ...
        端点, 总体 = 评测插补(按机场预测, 数据, 元信息, 私有配置, 配置, 异常计数)
    指标 = {"status": "completed", "task": 任务, "split": "val", "task_contract_version": 配置["task_contract_version"],
            "contract_sha256": 配置["task_contract_sha256"], "data_release": 配置["data_release"],
            "metric_scale": 配置["metric_scale"],
            "forecast_protocol": 配置["forecast_protocol"] if 任务 == "forecast" else None,
            "trial": {键: 元信息[键] for 键 in sorted(必填试验字段)}, "endpoints": 端点, "overall": 总体,
            "decision_policy_metrics": 决策指标别名(总体, 配置, 任务), "anomaly_counts": 异常计数}
    return 指标, 构造清单(参数, 私有配置, 配置, len(私有编号), 任务)
