...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "O2 事件加权数据前置：B3 冻结交付的 train-only 事件标志 CSV。"
            "缺省时训练路径与旧版逐字节一致；提供时 train batch 携带 "
            "event_mask/event_available，基线损失忽略之（行为不变）。"
        ),
    )
...
class AirportForecastDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        # O2 事件加权数据前置（train-only；B3 冻结交付 o2_hazard_event_flags_train_v1）：
        # event_flags[airport] = (event (rows,3) bool, avail (rows,3) bool)，按 source_index 对齐
        self.event_flags = event_flags.get(airport) if event_flags else None
    ...
    def __getitem__(self, item: int) -> dict[str, Any]:
        ...
        if self.event_flags is not None:
            event, avail = self.event_flags
            src = int(self.source_index[anchor])
            sample["event_mask"] = torch.from_numpy(event[src].copy())
            sample["event_available"] = torch.from_numpy(avail[src].copy())
        return sample
...
def load_o2_event_flags(csv_path: Path) -> dict:
    """读 B3 冻结交付的事件标志 CSV → {airport: (event (rows,3) bool, avail (rows,3) bool)}。

    行按 source_index 对齐（train split，全 anchor 覆盖）；口径 HAZARD-only
    （o2_event_slice_manifest_v1.json 为机器可读合同）。
    """
    import csv as _csv

    per_airport: dict[str, dict[int, tuple[list[bool], list[bool]]]] = {}
    with csv_path.open(encoding="utf-8-sig", newline="") as fh:
        for row in _csv.DictReader(fh):
            airport = row["airport"]
            src = int(row["source_index"])
            event = [row[f"is_hazard_event_T{h}"] == "True" for h in (1, 4, 8)]
            avail = [row[f"event_available_T{h}"] == "True" for h in (1, 4, 8)]
            per_airport.setdefault(airport, {})[src] = (event, avail)
    result: dict = {}
    for airport, rows in per_airport.items():
        n = max(rows) + 1
        event_arr = np.zeros((n, 3), dtype=bool)
        avail_arr = np.zeros((n, 3), dtype=bool)
        for src, (event, avail) in rows.items():
            event_arr[src] = event
            avail_arr[src] = avail
        result[airport] = (event_arr, avail_arr)
    return result
...
def train_epoch(
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        if "event_mask" in batch:
            loss = forecast_loss(
                prediction, target, node_mask,
                event_mask=batch["event_mask"].to(device, non_blocking=True),
                event_available=batch["event_available"].to(device, non_blocking=True),
            )
        else:
...
def main() -> None:
    ...
    event_flags = None
    if args.o2_event_flags is not None:
        if not args.o2_event_flags.is_file():
            raise FileNotFoundError(args.o2_event_flags)
        event_flags = load_o2_event_flags(args.o2_event_flags)

    train_parts = [
        AirportForecastDataset(args.data_root / "train", airport, event_flags=event_flags)
