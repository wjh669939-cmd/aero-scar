...
class AirportForecastDataset(Dataset):
    ...
    def __init__(self, split_root: Path, airport: str):
        ...
        for anchor, timestamp in enumerate(timestamp_ns):
            ...
            if all(index >= 0 for index in partner_row):
        ...
        for horizon_index, minutes in enumerate(HORIZONS.values()):
            actual = (
                timestamp_ns[self.partners[:, horizon_index]]
                - timestamp_ns[self.anchors]
            ) // NS_PER_MINUTE
        ...
        for horizon_index in range(len(HORIZONS)):
            partner_masks = np.asarray(
                self.runway_mask[self.partners[:, horizon_index]], dtype=bool
            )
            if not np.array_equal(anchor_masks, partner_masks):
    ...
    def __getitem__(self, item: int) -> dict[str, Any]:
        ...
        target = np.stack(
            [
                np.asarray(
                    self.runway[index, :, TARGET_INTERNAL_INDEX, 1:3],
                    dtype=np.float32,
                )
...
def main() -> None:
    ...
    final_val_loss, final_metrics, prediction_arrays = evaluate(
        model, val_loader, device, collect_predictions=True
    )
