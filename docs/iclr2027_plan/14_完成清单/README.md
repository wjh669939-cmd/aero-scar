...
| G-7 | **G1 执行器接线**：CLH trial → `aerowf_full_pipeline_v2.py` 子进程，成功判定按 D 手册 §6 六条件 | A+D | 🔶 8/27 执行器写好（`pipeline_executor.py`，六条件+失败线 11 项单测全绿），剩 D 机真跑 | 联调半天 |
| G-8 | **文件抽薄改造**：R/O 轴对象抽为 trial_features.py / trial_objective.py；抽薄后 seed43 复跑一致性验证 | D（A 出方案 ✅ 15 文档） | 🔶 方案已交 D，等执行 | 0.5 天 |
| G-9 | **部署**：CLH+contract_tools+C 私有配置 → D 的 3090 机器 | A+C | 🔶 部署清单已出（16 文档），等 D 机权限 | 0.5 天 |
| G-10 | **真实联调一轮**：真 checkpoint → predictions.npz → C evaluator → 全网格落盘 | A+C+D | ⬜ 唯一没走过的链路 | 联调半天 |
| G-11 | **D1 方差报告**（3 seed：42✅/43✅/2027⬜，冻结数据）→ decision_policy 数值标定 | D→A+C | 🔶 只差 seed 2027 一跑（~2.7h） | 8/27–8/28 |
| G-12 | DEC-001（M 轴）+ seeds v1.1 修订追认 + go/no-go 判定 | 导师+全组 | ⬜ 8/27–8/28 站会 | 会议项 |
| G-13 | 随机臂演练 2 trial，产物过 schema 校验 | A | ⬜ G-7 后半天 | 0.5 天 |
