...
>
> **组织调整（8/27 晚）**：D 侧任务并入实验侧。现分工 = 数据侧（B）+ 评测侧（C）+
> **实验侧 3 人**（分别负责 R / O / M 三条轴线开发，计划三台克隆 3090 并行）+ A 统筹兼实验侧成员。
> D 清单存量任务由实验侧接收；23 文档值守说明改为实验侧轮值。多机并行前置件：
> lineage 合并器（lineage_tools.py）已备；多机并行与轴档案方案见 **25 文档**（8/28：C 私有目录随镜像克隆，不再逐机重部署）。
